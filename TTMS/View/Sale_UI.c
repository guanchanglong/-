	...
	if (Play_Srv_FetchByID(playID, &buf))
	{
		...
		do {
			...
			case 't':
				...
				paging.totalRecords = Schedule_Srv_FetchByID(list, playID);
...
void Sale_UI_MgtEntry()
{
	...
	do {
		...
		printf("\n\t总数:%d\t\t\t\t\t\t\t\t\t\t%d/%d \n", paging.totalRecords, Pageing_CurPage(paging), Pageing_TotalPages(paging));
		printf("      ********************************************************************************************\n");
		printf("\t[P] 上一页    [N]下一页   [C]显示剧目演出计划   [S]查询剧目信息     [F]筛选剧目  [R]返回  \n");
		...
		case 'f':
			printf("\n\t请输入剧目名称过滤关键字：");
			scanf("%c",&filter);
			if (Play_Srv_FilterByName(head, filter) != NULL) {
...
void Sale_UI_ShowScheduler3(int playID)
{
	...
	if (Play_Srv_FetchByID(playID, &buf))
	{
		...
		do {
			...
			printf("\n\t\t\t[P]上一页      [N]下一页      [R]返回      [A]添加演出计划  \n\n\t\t\t[U]修改演出计划    [T]查找演出计划    [D]删除演出计划\n");
			printf("\n\n\t\t\t:");
			fflush(stdin);
			choice = getche();
			switch (choice)
