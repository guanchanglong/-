...
void  Play_UI_MgtEntry(int flag){  
	...
	else
	{
		do {
			...
			printf("\n\t\t\t总数:%2d\t\t\t\t\t\t    %d/%d \n",
				paging.totalRecords, Pageing_CurPage(paging), Pageing_TotalPages(paging));
			printf("\n\n\t\t\t[P]上页  [N]下页  [A]增加  [D]删除  [M]修改  [F]查询  [R]返回");
			printf("\n");
			printf("\n\t\t\t:");
			rewind(stdin);
			choice = getche();
...
int Play_UI_Add(void){
	...
	do {
		...
		int i = 0;
		rewind(fp);
		while (fread(&record, sizeof(play_t), 1, fp))
		{
			if (i < record.id)
			{
				i = record.id;
		...
		/*rec.id = EntKey_Perst_GetNewKeys("Play", 1);*/
...
int Play_UI_Modify(int id){
	...
	if (Play_Srv_Modify(&rec)) {
		...
		//	FILE* fpSour, * fpTarg;
		//	fpSour = fopen("PlayTmp.dat", "rb");
		//	if (NULL == fpSour) {
		//		printf("不能打开 %s!\n", "PlayTmp.dat");
		//	}

		//	fpTarg = fopen("Play.dat", "wb");
		//	if (NULL == fpTarg) {
		//		printf("不能打开 %s!\n", "Play.dat");
		//	}
