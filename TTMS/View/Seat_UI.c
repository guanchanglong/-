...
void Seat_UI_MgtEntry(int roomID) {
	...
	if (!seatCount) {		//若放映厅还没有设置座位，则自动生成座位
		...
		if (1) 
		{
			//在删除放映厅时，同时根据放映厅id删除座位文件中的座位
				if (rename("Studio.dat", "StudioTmp.dat") < 0) {
					printf("不能打开 %s!\n", "Studio.dat");
					return 0;
				}

				FILE* fpSour, * fpTarg;
				fpSour = fopen("StudioTmp.dat", "rb");
				if (NULL == fpSour) {
					printf("不能打开 %s!\n", "StudioTmp.dat");
					return 0;
				}

				fpTarg = fopen("Studio.dat", "wb");
				if (NULL == fpTarg) {
					printf("不能打开 %s!\n", "Studio.dat");
					return 0;
				}
				...
				while (!feof(fpSour)) {
					if (fread(&buf, sizeof(studio_t), 1, fpSour)) {
						if (studioRec.id == buf.id&&buf.seatsCount==0) {
							found = 1;
							continue;
						}
  ...
  do {
		...
		case '1':
			...
			break; 
		case '2':
			changedCount = Seat_UI_Delete(list, studioRec.rowsCount,studioRec.colsCount);
			if (changedCount > 0) {
				seatCount -= changedCount;
				//修改演出厅里的座位数量
				studioRec.seatsCount = seatCount;
