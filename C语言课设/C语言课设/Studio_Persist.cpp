...
*/
int Studio_Perst_Insert(studio_t *data) {	 	//储存新剧目 
	assert(NULL != data);

	////以下是新设计方案方案添加的代码
	////以下是新设计方案方案添加的代码
	long key = EntKey_Perst_GetNewKeys(STUDIO_KEY_NAME, 1); //为新演出厅分配获取。EntKey_Perst_GetNewKeys函数
	if (key <= 0)			//主键分配失败，直接返回
		return 0;
	data->id = key;		//赋给新对象带回到UI层。给放映厅确定新的编号 
	////以上是新设计方案方案添加的代码
	////以上是新设计方案方案添加的代码
